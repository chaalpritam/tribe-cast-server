...
HTTP server for storing and serving cast messages (posts, replies, reactions) for [Tribe Protocol](../tribe-protocol).
...
| GET | `/v1/castsByFid/:fid` | Paginated casts by FID |
| GET | `/v1/cast/:hash` | Single cast by hash |
| GET | `/v1/reactionsByTarget/:hash` | Reactions on a cast |
...
This starts Postgres + the cast server on port 3000.

### Manual

```bash
# Start Postgres
# Create database: tribe_casts
# Run migration:
...
2. **App key lookup** — signer must be a registered app key for the FID (cached from Solana)
